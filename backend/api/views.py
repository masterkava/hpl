...
from .serializers import (
    ...
    AuctionSerializer,
    AuctionPlayersDetailsSerializer,
    TeamSelectionSerializer,
    TeamAuthenticationSerializer
    )
...
class TeamAuthenticationList(APIView):
    """
    ...
    def get(self, request, format=None):
        ...
        auction_player_details = TeamAuthentication.objects.all()
        serializer = TeamAuthenticationSerializer(auction_player_details, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = TeamAuthenticationSerializer(data=request.data)
...
class TeamAuthenticationDetail(APIView):
    """
    Retrieve, update or delete a snippet instance.
    """
    def get_object(self, pk):
        try:
            return TeamAuthentication.objects.get(pk=pk)
        except Profile.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = TeamAuthenticationSerializer(auction_player_details)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = TeamAuthenticationSerializer(auction_player_details, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        auction_player_details.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
 


class TeamSelectionList(APIView):
    """
    List all snippets, or create a new snippet.
    """
    def get(self, request, format=None):
        auction_player_details = TeamSelection.objects.all()
        serializer = TeamSelectionSerializer(auction_player_details, many=True)
        return Response(serializer.data)
...
class TeamSelectionDetail(APIView):
    """
    Retrieve, update or delete a snippet instance.
    """
    def get_object(self, pk):
        try:
            return TeamSelection.objects.get(pk=pk)
        except Profile.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = TeamSelectionSerializer(auction_player_details)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = TeamSelectionSerializer(auction_player_details, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AuctionPlayersDetailsList(APIView):
    """
    List all snippets, or create a new snippet.
    """
    def get(self, request, format=None):
        auction_player_details = AuctionPlayersDetails.objects.all()
        serializer = AuctionPlayersDetailsSerializer(auction_player_details, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = AuctionPlayersDetailsSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class AuctionPlayersDetailsDetail(APIView):
    """
    Retrieve, update or delete a snippet instance.
    """
    def get_object(self, pk):
        try:
            return AuctionPlayersDetails.objects.get(pk=pk)
        except Profile.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = AuctionPlayersDetailsSerializer(auction_player_details)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        auction_player_details = self.get_object(pk)
        serializer = AuctionPlayersDetailsSerializer(auction_player_details, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AuctionDetailsList(APIView):
    """
    List all snippets, or create a new snippet.
    """
    def get(self, request, format=None):
        auction = Profile.objects.all()
